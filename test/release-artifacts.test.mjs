...
describe("portable CP/M release artifacts", () => {
  it("builds deterministic, fixed-slot CCP and BDOS binaries", async () => {
    const firstDirectory = await mkdtemp(join(tmpdir(), "portable-cpm-first-"));
    ...
    try {
      ...
      for (const component of first.components) {
        const firstBytes = await readFile(join(firstDirectory, component.file));
